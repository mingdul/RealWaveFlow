...
const TrackPage: React.FC<TrackPageProps> = () => {
  ...
  const loadApproveStems = async () => {
    ...
    try {
      ...
      const nonActiveStages = stages.filter(
      ...
      if (response.data) {
        ...
        console.error(
          '[ERROR][TrackPage] Failed to load stems:',
          response.message
        );
        setStems([]);
      }
  ...
  const loadStemsByVersion = async (version: number) => {
    ...
    try {
      ...
      console.error(
        '[ERROR][TrackPage] Error loading stems by version:',
        error
      );
      setStems([]);
    } finally {
  ...
  return (
    <div className='relative min-h-screen overflow-hidden bg-gradient-to-br from-stone-950 via-amber-950 to-neutral-950'>
      {/* 기하학적 네트워크 배경 */}
      <div className='absolute inset-0'>
        ...
        {/* 애니메이션 파티클들 */}
        <div className='absolute inset-0'>
          <div className='absolute left-1/4 top-1/4 h-1 w-1 animate-pulse rounded-full bg-amber-600 opacity-60'></div>
          <div className='absolute right-1/3 top-1/3 h-1 w-1 animate-pulse rounded-full bg-orange-700 opacity-40 delay-1000'></div>
          <div className='delay-2000 absolute bottom-1/4 left-1/3 h-1 w-1 animate-pulse rounded-full bg-stone-600 opacity-50'></div>
          <div className='delay-3000 absolute bottom-1/3 right-1/4 h-1 w-1 animate-pulse rounded-full bg-amber-600 opacity-60'></div>
          <div className='absolute left-1/2 top-1/2 h-1 w-1 animate-pulse rounded-full bg-orange-700 opacity-40 delay-500'></div>
        </div>
      </div>
