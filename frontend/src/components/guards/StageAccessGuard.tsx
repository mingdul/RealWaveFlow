...
interface StageAccessGuardProps {
  children: React.ReactNode;
  stageId: string;
}

const StageAccessGuard: React.FC<StageAccessGuardProps> = ({ children, stageId }) => {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      ...
      try {
        console.log('[StageAccessGuard] Checking access for stage:', stageId);
        const result = await trackAccessService.checkStageAccess(stageId);
        
        console.log('[StageAccessGuard] Access check result:', result);
        setHasAccess(result.hasAccess);
        setTrackId(result.trackId);
      } catch (error) {
        console.error('[StageAccessGuard] Access check failed:', error);
        setHasAccess(false);
      } finally {
        setAccessLoading(false);
      }
  ...
  }, [isAuthenticated, authLoading, stageId]);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-900">
        <div className="text-center">
          ...
          <p className="text-white">스테이지 접근 권한을 확인하는 중...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="text-center max-w-md mx-auto px-4">
          <div className="mb-8">
            ...
            <p className="text-gray-300 mb-6">
              이 스테이지에 접근할 권한이 없습니다. 트랙 소유자나 협업자만 접근할 수 있습니다.
            </p>
            <button
